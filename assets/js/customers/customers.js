...
// --- Helper: Clear all select2 filters ---
function clearSelect2Filters() {
    $('.select2-filter').val(null).trigger('change');
}

// --- Helper: Clear all header text filters ---
function clearHeaderTextFilters(exceptInput) {
    $('.table thead input[type="text"]').each(function() {
        if (this !== exceptInput) {
            this.value = '';
        }
    });
...
// --- Patch filterTable to clear select2 filters when used ---
window.filterTable = function(input, columnIndex) {
    // When typing in header input, clear select2 filters
    clearSelect2Filters();
    ...
    let searchColumn = '';
    switch (columnIndex) {
        case 1: searchColumn = 'name'; break;
        case 2: searchColumn = 'phone1'; break;
        case 3: searchColumn = 'owed_amount'; break;
        case 4: searchColumn = 'advance_payment'; break;
        case 5: searchColumn = 'city'; break;
        case 6: searchColumn = 'location'; break;
        case 7: searchColumn = 'customer_type_name'; break;
...
// --- Patch select2 filters to clear header text filters and do AJAX search ---
$(document).ready(function() {
    // ... existing code ...
    // Patch select2 filters for AJAX search
    $('.select2-filter').on('change', function() {
        // Clear header text filters
        clearHeaderTextFilters();
        // Build search params
        let params = [];
        $('.select2-filter').each(function() {
            ...
            if (val && col) {
                ...
                switch (col) {
                    ...
                    case 5: colName = 'city'; break;
                    case 7: colName = 'customer_type_name'; break;
                }
                if (colName) {
                    params.push(`${encodeURIComponent(colName)}=${encodeURIComponent(val)}`);
                }
            }
        });
        if (params.length === 0) {
        ...
        // Only one filter at a time (for simplicity)
        const url = `../process/customers/select.php?${params[0] ? 'search_column=' + params[0].split('=')[0] + '&search_value=' + params[0].split('=')[1] : ''}`;
        fetch(url)
            .then(response => response.json())
            .then(data => {
                if (data.success) {
